...
# 计算百分比差异
df['市盈率估值买点差异'] = ((df['最新价'] / df['市盈率估值买点'] - 1) * 100).round(2)
df['净利润估值买点差异'] = ((df['最新价'] / df['净利润估值买点'] - 1) * 100).round(2)

# 将数值转换为字符串并追加到原列中
df['市盈率估值买点'] = df['市盈率估值买点'].astype(str) + ' | ' + df['市盈率估值买点差异'].astype(str) + '%'
df['净利润估值买点'] = df['净利润估值买点'].astype(str) + ' | ' + df['净利润估值买点差异'].astype(str) + '%'
